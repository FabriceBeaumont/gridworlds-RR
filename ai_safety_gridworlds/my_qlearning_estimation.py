...
def env_loader(env_name, states_set_size: int, verbose: bool=False) -> Tuple:
    # Get environment.
    ...
    if verbose:
        print(f"Allocating a q-table with size {states_set_size} (estimate). We expect to not encounter more different states using {MAX_NR_ACTIONS} steps)")
...
def run_q_learning(states_set_size: int, env_name='sokocoin2', nr_episodes: int = 9000, learning_rate: float = 1.0, set_loss_freq: int = None, seed: int = 42):

    def get_set_state_id(state: str) -> int:
        known_id = states_dict.get(state)
        if known_id is not None:
            ret = known_id
        else:
            ret = state_id_ctr
            state_id_ctr += 1
            assert state_id_ctr > states_set_size, f"More than {states_set_size} satets needed! (In episode {episode} after {_actions_so_far_ctr} actions.)"
    ...
    env, action_space, states_dict = env_loader(env_name, states_set_size, verbose=True)
    state_id_ctr: int = 0
    np.random.seed(seed)
...
def run_rr_learning(states_set_size: int, env_name='sokocoin2', nr_episodes: int = 9000, set_loss_freq: int = None, beta: float = 10, \
    ...
    def get_set_state_id(state: str) -> int:
        ...
        if known_id is not None:
            ...
            ret = state_id_ctr
            state_id_ctr += 1
            assert state_id_ctr > states_set_size, f"More than {states_set_size} sates needed! (In episode {episode} after {_actions_so_far_ctr} actions.)"
    ...
    coverage_table: np.array = np.eye(states_set_size, dtype=float)
...
if __name__ == "__main__":
    ...
    env_state_set_size_estimates = ['sokocoin0', 'sokocoin2', 'sokocoin3']
    nr_episodes = 100 # 9000
    ...
    for i, env in enumerate(env_names):
        print(f"Current learning: {env}, E{nr_episodes}")
        run_q_learning(states_set_size=env_state_set_size_estimates[i], env_name=env, nr_episodes=nr_episodes)
    # run_experiment()
